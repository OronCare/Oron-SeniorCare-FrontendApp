...
import { mockAlerts, mockBranches } from '../../mockData';
import { useAuth } from '../../context/AuthContext';
export const FacAdminNotifications = () => {
  const { user } = useAuth();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            System alerts and updates across all branches.
          </p>
        </div>
      ...
      <Card noPadding>
        <div className="border-b border-slate-200 px-2">
          <nav className="flex gap-2 overflow-x-auto">
            ...
            <button
              ...
              className={`py-4 px-4 text-sm font-medium border-b-2 whitespace-nowrap transition-colors ${activeTab === tab ? 'border-brand-500 text-brand-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'}`}>
        ...
        <div className="divide-y divide-slate-100">
          {filteredAlerts.map((alert) => {
            ...
            myBranches.find((b) => b.id === alert.branchId) :
            null;
            return (
              <div
                key={alert.id}
                className={`p-4 sm:p-6 flex flex-col sm:flex-row gap-4 transition-colors ${alert.status === 'Unread' ? 'bg-brand-50/30' : 'hover:bg-slate-50'}`}>
                
                <div
                  className={`shrink-0 h-10 w-10 rounded-full flex items-center justify-center ${getBgColor(alert.severity)}`}>
                ...
                <div className="flex-1 space-y-1">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <h3
                      className={`text-base font-semibold ${alert.status === 'Unread' ? 'text-slate-900' : 'text-slate-700'}`}>
                  ...
                  <p
                    className={`text-sm ${alert.status === 'Unread' ? 'text-slate-700' : 'text-slate-500'}`}>
                  ...
                  <div className="flex items-center gap-3 pt-2 flex-wrap">
                    ...
                    {alert.healthState &&
                    <span
                      className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${getHealthStateColor(alert.healthState)}`}>
